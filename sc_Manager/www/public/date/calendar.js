...
Calendar = function (mondayFirst, dateStr, onSelected, onClose) {
	...
	this.isPopup = true;
	this.weekNumbers = false;//true;//Ҫǰ
...
Calendar.dayMouseOver = function(ev) {
	...
	if (el.ttip) {
		if (el.ttip.substr(0, 1) == "_") {
			...
			el.ttip = ""+date.print("D")+"  " +date.print("y")+ " "+date.print("M")+date.print("d") +"";
		}
...
Calendar.cellClick = function(el) {
	...
	if (typeof el.navtype == "undefined") {
		...
		cal.callCloseHandler();//Ҳùر;added by jarry
	} else {
		if (el.navtype == 200) {
			Calendar.removeClass(el, "hilite");
...
//	(this.weekNumbers) && ++title_length;
	...
	this._nav_py = hh("<img id=objyprev src=\""+roothtml+"/public/date/img/yprev.gif\">", 1, -2);
...
//	this._nav_py = hh("&#x00ab;", 1, -2);
	...
	//	this._nav_ny = hh("&#x00bb;", 1, 2);
	this._nav_ny = hh("<img id=objynext src=\""+roothtml+"/public/date/img/ynext.gif\">", 1, 2);
	this._nav_ny.width = 25;
	this._nav_ny.ttip = Calendar._TT["NEXT_YEAR"];

//	this._nav_pm = hh("&#x2039;", 1, -1);
	this._nav_pm = hh("<img id=objmprev src=\""+roothtml+"/public/date/img/mprev.gif\">", 1, -1);
...
//	this._nav_nm = hh("&#x203a;", 1, 1);
	this._nav_nm = hh("<img id=objmnext src=\""+roothtml+"/public/date/img/mnext.gif\">", 1, 1);
...
//	this._nav_now.ttip = Calendar._TT["GO_TODAY"];
	...
	if (this.isPopup) {
		...
		//this.title.style.cursor = "move";//עƶ״̬
		//hh("&#x00d7;", 1, 200).ttip = Calendar._TT["CLOSE"];//ȥرշͼƬ;
		var td_close = hh("<img id=objclose src=\""+roothtml+"/public/date/img/close.gif\">", 1, 200);
		td_close.ttip = Calendar._TT["CLOSE"];
		td_close.align = "right";
		//hh("#<div><img src=\"/image/calendar/close.gif\" onmousedown=\"return;\"></div>", 1, 200).ttip = Calendar._TT["CLOSE"];
	}	
...
//	this.title.firstChild.data = year + "" + Calendar._MN[month];
...
Calendar.prototype._dragStart = function (ev) {
	...
	with (Calendar) {
		//addEvent(document, "mousemove", calDragIt);//ע϶
