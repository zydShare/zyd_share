...
var account = require('./fgs_openCardLog.js');
//单条显示
...
module.exports.form = function(f, pg, mo) {
	...
	if (f.data.id != '') {
		...
		sql = "update 分_开卡日志表 set 套餐id='" + f.data.套餐id +
			"', 账号='" + f.data.账号 +
			"', 姓名='" + f.data.姓名 +
			"', 卡号='" + f.data.卡号 +
			"', 支付方式='" + f.data.支付方式 +
		...
		sql = "insert into 分_开卡日志表 (套餐id,账号,姓名,卡号,支付方式,上级,上级姓名,套餐,面值,赠送,日返积分,总额,数量,支付订单号,类别,状态,录入人,录入时间,内容,备注,支付时间,社区代理分公司名称,市代理分公司名称,市代理分公司id,社区代理分公司id,商户订单号 ) values ('" +
			f.data.套餐id + "','" +
			f.data.账号 + "','" +
			f.data.姓名 + "','" +
			f.data.卡号 + "','" +
