...
module.exports.run = function(f, pg, mo) {
	...
	if(f.data != null) {
		...
		if(f.data.二级标题 != null && f.data.二级标题 != ''){
			where += " and  二级标题 = '" + f.data.二级标题 + "'";
		}		
	...
	var str =JSON.stringify(f.data);
	 if(str.length == 2){
         p.sql = "select id,二级标题,二级副标题,图片id,图标,图片,服务账号id,图片链接,排序,状态,录入人,录入时间  from 平_二级图片表  where "+ where;
  	}else{
  		p.sql = "select id,二级标题,二级副标题,图片id,图标,图片,服务账号id,图片链接,排序,状态,录入人,录入时间  from 平_二级图片表   where "+ where;
  	}	
