...
module.exports.run = function(f, pg, mo) {
	...
	/*var str =JSON.stringify(f.data);
	 if(str.length == 2){
         p.sql = "select id,账号,银行卡姓名,手机号,银行卡号,银行名,身份证号,证件类型,状态,类别,支行名称,兑换类型,兑换时限,兑换更改时限,审核状态,省,市,录入人,录入时间   from 平_银行信息表    where " + where;
  	}else{*/
  		p.sql = "select id,账号,银行卡姓名,手机号,银行卡号,银行名,身份证号,证件类型,状态,类别,支行名称,兑换类型,兑换时限,兑换更改时限,审核状态,省,市,录入人,录入时间,图片  from 平_银行信息表  where "+where;
  	//}
	//做一个判断引用哪个数据库，放到f里面
