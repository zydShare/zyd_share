...
module.exports.run = function(f, pg, mo) {
	...
	if(f.data != null) {
		...
		if(f.data.二级分类名称 != null && f.data.二级分类名称 != ''){
			where += " and  二级分类名称 = '" + f.data.二级分类名称 + "'";
		}
	...
	var str =JSON.stringify(f.data);
	 if(str.length == 2){
         p.sql = "select id,二级分类名称,图标,类别id,状态,录入人,录入时间,备注  from 商_二级分类表  where "+ where;
  	}else{
  		p.sql = "select id,二级分类名称,图标,类别id,状态,录入人,录入时间,备注  from 商_二级分类表   where "+ where;
  	}	
