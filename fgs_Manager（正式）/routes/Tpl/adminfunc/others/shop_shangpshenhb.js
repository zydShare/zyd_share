...
module.exports.run = function(f, pg, mo) {
	...
	if(f.data != null) {
		...
		if(f.data.作废原因 != null && f.data.作废原因 != ''){
			where += "and 作废原因 like '%"+f.data.作废原因+"%'";
		}
		if(f.data.作废时间 != null && f.data.作废时间 != ''){
			where += "and 作废时间 like '%"+f.data.作废时间+"%'";
		}
	...
	var str =JSON.stringify(f.data);
	 if(str.length == 2){
         p.sql = "select id,商品名称,作废原因,作废时间,状态,录入人,录入时间,备注   from 商_商品审核表  where " + where;
  	}else{
  		p.sql = "select id,商品名称,作废原因,作废时间,状态,录入人,录入时间,备注   from 商_商品审核表  where  "+where;
  	}
	//做一个判断引用哪个数据库，放到f里面
