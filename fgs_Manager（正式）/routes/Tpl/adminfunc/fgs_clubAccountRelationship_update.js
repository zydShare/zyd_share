...
var account = require('./fgs_clubAccountRelationship.js');
//单条显示
...
module.exports.form = function(f, pg, mo) {
	...
	if (f.data.id != '') {
		...
		sql = "update 分_俱乐部账户关系表 set 俱乐部名称='" + f.data.俱乐部名称 +
			"', 发放部门='" + f.data.发放部门 +
			"', 状态='" + f.data.状态 +
			"', 类别='" + f.data.类别 +
			"', 录入人='" + f.data.录入人 +
		...
		sql = "insert into 分_俱乐部账户关系表 (俱乐部名称,发放部门,状态,类别,录入人,录入时间,备注,俱乐部id ) values ('" +
			f.data.俱乐部名称 + "','" +
			f.data.发放部门 + "','" +
			f.data.类别 + "','" +
			f.data.状态 + "','" +
