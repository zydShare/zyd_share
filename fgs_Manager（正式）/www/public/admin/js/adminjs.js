...
function seach_html(_seach_url, html) {
	...
	arr_b.forEach(function(name, key) {
		if(name == null || name == '') {} else if(name.indexOf('_seach=') >= 0) {} else {
...
function howid() {
	...
	if(id) {
		...
		$(".handle-btns .img-1 input").attr({
			"disabled": ""
		});
		$(".handle-btns .img-2 input").attr({
			"disabled": "disabled"
		});
...
function onb(inp) {
	...
	if(samp != input) {
		...
		htmlobj = $.ajax({
			url: jqtexthtml + "/name/" + thinkname + "/id/" + id + '/con/' + con + '/control/' + control,
			async: false
		});
...
function jq_bottom(type, html, name, pass, mo) {
	...
	if(type == 1) {
		if(idinput_num == ''){
			console.log(idinput_num+"000");
			id = $(".idinput[checked=true]:last").val();
		}else{
			console.log(idinput_num+"222");			
			id = idinput_num;
		}

		id_con = "id=" + id + "&how_id=" + how_id;
		console.log(id_con+"aa");			
		
		html_con = "&_b_name=" + name + "&_b_pa=" + pass + "&_b_f=" + mo;
		
		console.log(html_con+"bb");			
		...
		if(id)
			id_con = "&id=" + how_id;
		$.ajax({
			url: app + "&_ms_main=no&_f=_button_" + html + id_con,
			success: function(r) {
				alert(r);
				if(r == '提交成功') {
					relo('')
		...
		if(id)
			id_con = "&id=" + how_id;
		$.ajax({
			url: html + '&_name=' + name + id_con + "&_b_pa=" + pass,
			success: function(r) {
				alert(r);
				if(r == '提交成功') {
					relo('');
...
function jq_show_img(name) {
	$(".jqclass").hover(function() {
		...
		if(!$(this).find("img").attr("src")) {
			$.ajax({
				url: app + "/adminsys/jqimg/name/" + name + "/id/" + id + "/img_name/" + img_name,
...
function how_img_t(type, name) {
	if(type == 2 && $("#h_i_" + name + "2").html() == "") {
		...
		$.ajax({
			url: app + "/adminsys/how_img/href_con/" + href_con,
			success: function(r) {
				$("#h_i_" + name + "2").html(r);
				how_img_de();
			}
	...
	$("#h_i_" + name + "2").hide();
	$("#h_i_" + name + type).show();
	$(".h_i_t_" + name + " input").attr({
		"class": "reset_btn"
	});
	$(".h_i_t_" + name + " #botton" + type).attr({
		"class": "submit_btn"
	});
...
function how_img_de() {
	$(".how_img_u #a input").click(function() {
		if($(this).attr("checked") == true) {
			...
			$(this).parent().parent().find(".d input").attr({
				"checked": true
			});
		} else {
			$(this).parent().parent().find(".d input").attr({
	...
	$(".how_img_u #a a").click(function() {
		if(confirm('确定要删除吗！')) {
			...
			if(id) {
				...
				$.ajax({
					url: app + "/adminsys/how_img_de/conditions/" + conditions + "/id/" + id,
					success: function(r) {
						alert("删除成功");
					}
				})
...
function select_button() {
	$(".select_button a").click(function() {
		...
		if(v[0] == 1) {
			...
			id_con = "&id=" + id;
			$.ajax({
				url: app + "?_ms_main=no&_f=_button_" + v[1] + id_con + "&_b_pa=" + v[3],
				success: function(r) {
					alert(r);
					if(r == '提交成功') {
						relo('')
...
if(accessKeyId == null || accessKeyId == null || bucket == null || region == null) {
	...
	$.ajax({
		url: "?_m=admin&_f=file_parameter",
		success: function(r) {
		...
		success: function(data) {
			...
			if(data.状态 == '成功') {
				...
				$(".delete_pic").click(function() {
					if(confirm('确定要删除吗，删除后不可恢复！')) $(this).parent().remove();
				});
			...
			success: function(data, status) {
				...
				if(data.状态 == '成功') {
					...
					if(type == 'img' || type == 'ali') {
						...
						$(".download_logo").css("display", "block");
