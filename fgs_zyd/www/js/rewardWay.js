...
$("#confirmBtn").on('click', function() {
	...
	if($('#switchIcon').attr('data') != "selected") {
		...
		if(tj == 'true') {
			layObj.lay_input = '<div class="content" style="margin: 0 auto; padding-top: .15rem; display: table;"><input type="password" id="fgs-password" class="fgs-password" minlength="6" maxlength="8" onclick="shuru2(this);" onkeyup="shuru2(this);" placeholder="请您输入分公司密码" /></div>';
					...
					$.ajax({
						...
						success: function(s) {
							...
							if(s.状态 != "成功") {
								...
								layer.close(index);
								console.log(layObj.pic)
								layObj.pic = "ic_type_success.png";
								layObj.lay_content = "切换成功";
								layObj.reload = function() { history.go(-1); }
			...
			layObj.content_1("分公司密码", layObj.lay_input, layObj.T_button,layObj.F_button);


		} else {
			layObj.lay_content = "您在" + $("#lastTime").html() + "已进行修改操作，</br>" + $("#confirmBtn").attr("data-day") + "天后方可进行修改";
			layObj.reload = function() { history.go(0); }
