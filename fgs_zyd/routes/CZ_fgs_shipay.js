...
/*业务处理*/
	...
	{
		console.log("assadasdasdasdasas");
		var 开=result.数据[0];
		f.申请id=开.id;
		f.分公司编号 = 开.编号;
		f.分_名称 = 开.名称;
 ...
 /*交互分公司数据*/	
	sql = "select sum(股数) as 总持股数  from 分_分公司申请表  where 账号 = '" + f.账号 + "' and 编号 = '" + f.分公司编号 + "' and 状态 = '已支付'";
...
if(f.开_类别 == '创建市代理分公司')
{
	console.log("aaaaaaaaaaaaaaaaaa1232423423423");
    var types = ['appid','filiale_No','filiale_Id','onlyID'];
    var r = {};
    r.func = 'p_ins_filiale';			
    r.mch_id = server.mch_id;
    r.filiale_Type='市代理分公司';
...
/*---------*/		
		...
		{
			sql = "insert into 分_分公司成员表( id,账号, 姓名, 分公司id, 所占股数, 单股金额, 分公司名称, 状态, 类别, 录入人, 录入时间, 备注 )values('" + f.分公司成员表_id + "','" + f.手机号 + "', '" + f.开_姓名 + "', '" + f.分公司编号 + "', '" + f.开_股数 + "', '" + f.分_设置单股金额 + "', '" + f.分_名称 + "', '正常', '股东', '系统', '" +时间+ "', '')";
		...
		{
			f.所占股数=result6.数据[0].所占股数;
			f.总份数 = Number(f.所占股数) + Number(f.开_股数);
...
/*---------*/		
	...
	if(f.开_类别 == '创建市代理分公司')
	{
		console.log("aaaaaaaaaaaaaaaaaa1234");
...
/*创建分公司提成录入*/
	if(Number(f.分_设置全球提成) != 0 &&  f.开_类别 =='创建市代理分公司')
	{
		sql = "insert into 分_基本日志表( 账号, 姓名,当时类别, 卡号, 卡姓名, 应发积分, 实发积分, 开卡id, 数量,套餐, 支付方式, 状态, 类别, 录入人, 录入时间, 备注 )values('" + f.全俱乐部id + "', '" + f.全俱乐部名称 + "','', '" + f.分公司编号 + "', '" + f.分_名称 + "', " + f.全球提成 + ", " + f.全球提成 + ", " + f.开_id + "," + f.开_股数 + ", '创建市代理分公司提成', '" + f.r_支付方式 + "', '未结算', '分公司提成_全球', '', '" + 时间 + "', '')";
		console.log(sql);
	...
	if(Number(f.分_设置七大提成) != 0 &&  f.开_类别 =='创建市代理分公司')
	{
		sql = "insert into 分_基本日志表( 账号, 姓名,当时类别, 卡号, 卡姓名, 应发积分, 实发积分, 开卡id, 数量,套餐, 支付方式, 状态, 类别, 录入人, 录入时间, 备注 )values('" + f.七大俱乐部id + "', '" + f.七大俱乐部名称 + "','', '" + f.分公司编号 + "', '" + f.分_名称 + "', " + f.七大提成 + ", " + f.七大提成 + ", " + f.开_id + "," + f.开_股数 + ", '创建市代理分公司提成', '" + f.r_支付方式 + "', '未结算', '分公司提成_七大', '', '" + 时间 + "', '')";
		console.log(sql);
	...
	if(Number(f.分_设置上级提成) != 0 && (f.上级 !='' && f.上级 !=null && f.上级 !=undefined) &&  f.开_类别 =='创建市代理分公司')
	{
		sql = "insert into 分_基本日志表( 账号, 姓名,当时类别, 卡号, 卡姓名, 应发积分, 实发积分, 开卡id, 数量,套餐, 支付方式, 状态, 类别, 录入人, 录入时间, 备注 )values('" + f.上级 + "', '" + f.上级姓名 + "','', '" + f.分公司编号 + "', '" + f.分_名称 + "', " + f.上级提成 + ", " + f.上级提成 + ", " + f.开_id + "," + f.开_股数 + ", '创建市代理分公司提成', '" + f.r_支付方式 + "', '未结算', '分公司上级提成', '', '" + 时间 + "', '')";
		console.log(sql);
...
/*加入分公司提成录入*/	
	if(Number(f.分_设置全球提成) != 0 &&  f.开_类别 =='加入市代理分公司') 
	{
		sql = "insert into 分_基本日志表( 账号, 姓名,当时类别, 卡号, 卡姓名, 应发积分, 实发积分, 开卡id, 数量,套餐, 支付方式, 状态, 类别, 录入人, 录入时间, 备注 )values('" + f.全俱乐部id + "', '" + f.全俱乐部名称 + "','', '" + f.分公司编号 + "', '" + f.分_名称 + "', " + f.全球提成 + ", " + f.全球提成 + ", " + f.开_id + "," + f.开_股数 + ", '加入市代理分公司提成', '" + f.r_支付方式 + "', '未结算', '分公司提成_全球1111', '', '" + 时间 + "', '')";
	...
	if(Number(f.分_设置七大提成) != 0 &&  f.开_类别 =='加入市代理分公司')
	{
		sql = "insert into 分_基本日志表( 账号, 姓名,当时类别, 卡号, 卡姓名, 应发积分, 实发积分, 开卡id, 数量,套餐, 支付方式, 状态, 类别, 录入人, 录入时间, 备注 )values('" + f.七大俱乐部id + "', '" + f.七大俱乐部名称 + "','', '" + f.分公司编号 + "', '" + f.分_名称 + "', " + f.七大提成 + ", " + f.七大提成 + ", " + f.开_id + "," + f.开_股数 + ", '加入市代理分公司提成', '" + f.r_支付方式 + "', '未结算', '分公司提成_七大1111', '', '" + 时间 + "', '')";
	...
	if(Number(f.分_设置上级提成) != 0 && (f.上级 !='' && f.上级 !=null && f.上级 !=undefined) &&  f.开_类别 =='加入市代理分公司')
	{
		sql = "insert into 分_基本日志表( 账号, 姓名,当时类别, 卡号, 卡姓名, 应发积分, 实发积分, 开卡id, 数量,套餐, 支付方式, 状态, 类别, 录入人, 录入时间, 备注 )values('" + f.上级 + "', '" + f.上级姓名 + "','', '" + f.分公司编号 + "', '" + f.分_名称 + "', " + f.上级提成 + ", " + f.上级提成 + ", " + f.开_id + "," + f.开_股数 + ", '加入市代理分公司提成', '" + f.r_支付方式 + "', '未结算', '分公司上级提成', '', '" + 时间 + "', '')";
	...
	f.日返积分2=Number(f.开_股数) * Number(f.全日返积分) * Number(f.每股赠送个数);
	...
	/*sql = "insert into 分_全返套餐表(套餐名称, 卡号, 账号, 姓名, 手机号,数量, 消费账户, 赠送积分, 面值, 日返还积分, 上次返还时间, 状态, 类别, 录入人, 录入时间, 备注 )values('"+f.全名称+"', " + f.全返套餐表_id + ", '" + f.手机号 + "', '" + f.开_姓名 + "', '" + f.手机号 + "','" + f.全球套餐数量 + "', '" + f.全球套餐消费账户 + "', '" + f.全球套餐消费账户 + "', 2339, '" + f.日返积分2 + "', '" + 时间 + "', '正常', '分公司套餐', '系统', '" + 时间 + "', '')";
	console.log(sql);
