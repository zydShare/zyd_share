/**
 * 创建人：唐鹏程
 * 创建时间：2017年6月19日14:41:52
 * 创建内容：查询所在的社区代理分公司列表
 */

/*
func=CZ_fgs_shequ_default
...
module.exports.run = function (body, pg, mo) {
	...
	//查询社区代理分公司列表
	sql = "select a.分公司id, a.分公司名称 from 分_分公司成员表 a,分_分公司表 b where a.账号 = '" + f.账号 + "' and a.状态 = '正常' and (b.状态 = '状态' or b.状态 = '临时') and b.类别 = '社区代理分公司' and a.分公司id = b.编号 order by a.录入时间 desc limit " + 每页条数 + " offset " + 开始条数;
	...
	if (result.数据.length != 0) {
		//查询是否设置默认分公司
		sql = "select 社区分公司 from 分_默认表 where 状态 = '正常' and 唯一id = '" + f.onlyID + "'";
		result = pgdb.query(pg, sql);
		if (result.数据.length == 0) {
			f.默认分公司编号 = '';
			f.默认分公司名称 = '';
