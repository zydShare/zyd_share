...
/*查询分公司套餐的股数和单股金额*/
	...
	else
	{
		...
		f.套餐id=result4.数据[0].id;
	}
	
/*限制分公司创建数量*/
	sql = "select COUNT(id) as 社区id from 分_分公司表 where 市 = '" + f.选择城市 + "' and 类别 ='社区代理分公司'";
	var result5=pgdb.query(pg,sql);
	if(result5.数据.length == 0)
	{
		f.社区代理分公司数量='';
	}
	f.社区代理分公司数量=result5.数据[0].社区id;
	if(f.社区代理分公司数量 > f.分公司创建数量)
	{
		p.状态 = '该区域的社区代理分公司数量已满,请选择其他的社区';
		return p;
	}
...
/*判断推荐人是否是分公司股东或者创始人*/
	...
	if(f.推荐分公司编号 !='' && f.推荐分公司编号 !=null && f.推荐分公司编号 != undefined)
	{
		sql = "select id,名称  from 分_分公司表 where 编号 = '" + f.推荐分公司编号 + "'";		
...
/*-----------------------------*/
    ...
    r.notify_url = server.pay_return_url + "/CZ_fgs_shequpay.xhtml";
...
/*创建市级代理分公司下单*/
	...
	sql = "insert into 分_分公司申请表(账号, 姓名, 名称, 编号, 省, 市,区,商户订单号,上级,上级姓名,分公司设置id,分公司设置名称, 层1分公司名称, 层1分公司id, 层2分公司名称, 层2分公司id, 层3分公司名称, 层3分公司id, 金额, 股数, 支付方式, 支付时间, 支付订单号, 状态, 类别, 录入人, 录入时间, 备注, 分公司密码 ,每股赠送个数)values('" + f.账号 + "', '" + f.昵称 + "', '" + f.分公司名称 + "', '" + f.分公司编号 + "', '" + f.选择省份 + "', '" + f.选择城市 + "','" + f.选择区 + "','"+f.商户订单号+"','" + f.推荐人手机号 + "','" + f.推荐人姓名 + "', '" + f.分公司设置id + "', '" + f.分公司类型 + "', '" + f.推名称 + "', '" + f.推id + "', '','0', '','0', " + f.金额 + ", '" + f.股数 + "', '', '', '', '未支付', '创建社区代理分公司', '系统', '" + 时间 + "', '', '" + f.分公司密码 + "',"+f.每股赠送个数+")";
