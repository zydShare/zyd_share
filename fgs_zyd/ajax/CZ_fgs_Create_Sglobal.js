...
module.exports.run = function (body, pg, mo) {
    ...
    var p = {};
	p.状态 = "成功";
//	console.log(body.data,body.receive)
	...
	if(f.认购股份 <= 0 || f.认购股份 > 100 )
	{	
		p.状态 = '认购股份不能小于1股或者大于100股,请重新输入';
		return p;
	}
...
/*查询分公司套餐的股数和单股金额*/
	sql = "select id,名称,Round(@单股金额,2) as 单股金额,股数,创建数量,每股赠送个数  from 分_分公司设置表  where id = '" + f.分公司设置id + "'";
...
/*限制分公司创建数量*/
	sql = "select COUNT(id) as 市id from 分_分公司表 where 市 = '" + f.选择城市 + "' and 类别 ='独家代理分公司'";
	var result5=pgdb.query(pg,sql);
	if(result5.数据.length <= 0)
	{
		f.独家代理分公司数量='';
	}
	f.独家代理分公司数量=result5.数据[0].市id;
	if(f.独家代理分公司数量 > f.分公司创建数量)
	{
		p.状态 = '该区域的独家代理分公司数量已满,请选择其他的市';
		return p;
	}
...
/*-----------------------------*/
    ...
    r.notify_url = server.pay_return_url + "/CZ_fgs_DJpay.xhtml";
...
/*创建市级代理分公司下单*/
	...
	sql = "insert into 分_分公司申请表(账号, 姓名, 名称, 编号, 省, 市,区,商户订单号,上级,上级姓名,分公司设置id,分公司设置名称, 层1分公司名称, 层1分公司id, 层2分公司名称, 层2分公司id, 层3分公司名称, 层3分公司id, 金额, 股数, 支付方式, 支付时间, 支付订单号, 状态, 类别, 录入人, 录入时间, 备注, 分公司密码,每股赠送个数 )values('" + f.账号 + "', '" + f.昵称 + "', '" + f.分公司名称 + "', '" + f.分公司编号 + "', '" + f.选择省份 + "', '" + f.选择城市 + "','','"+f.商户订单号+"','" + f.推荐人手机号 + "','" + f.推荐人姓名 + "', '" + f.分公司设置id + "', '独家代理分公司', '', 0, '',0, '',0, " + f.金额 + ", '" + f.股数 + "', '', '', '', '未支付', '创建独家代理分公司', '系统', '" + 时间 + "', '', '" + f.分公司密码 + "',"+f.每股赠送个数+")";
