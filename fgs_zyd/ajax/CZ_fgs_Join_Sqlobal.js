...
module.exports.run = function (body, pg, mo) 
{
	...
	if(Number(f.股数) <= 0 || Number(f.股数) > 100)
	{	
		p.状态 = '您选择的股数不能小于1股或者大于100股份,请重新输入';
		return p;
	}
...
/*业务处理模块*/	
	...
	sql ="select id from 分_分公司表  where  编号='"+f.分公司编号+"' and 类别='独家代理分公司'";
	var result = pgdb.query(pg, sql);
	if(result.数据.length == 0)
	{
		p.状态 = '您输入的分公司编号不是独家代理分公司，请重新输入';
		return p;
	}
	...
	if(Number(result5.数据[0].已分配股数))
	{
		...
		{
			p.状态='分公司剩余股份'+Number(f.剩余股数)+',您最多可以购买'+	Number(f.剩余股数)+'股，请重新输入!';
			return p;
		}
	}
	...
	if(计算单个成员股数 >=101)
	{
		p.状态='抱歉，您所拥有的总股份数不能超过100股，请重新输入股份数量！';
		return p;
	}
	else
	{
...
/*加入市级代理分公司下单*/
	sql="INSERT INTO 分_分公司申请表(账号, 姓名, 名称, 编号, 省, 市, 区,商户订单号, 分公司设置id, 分公司设置名称,上级, 上级姓名, 层1分公司名称, 层1分公司id, 层2分公司名称, 层2分公司id, 层3分公司名称, 层3分公司id,金额, 股数, 支付方式, 支付时间, 支付订单号, 每股赠送个数, 状态, 类别, 录入人, 录入时间, 备注, 密码, 分公司密码)VALUES ('"+f.账号+"', '"+f.昵称+"', '"+f.分公司名称+"', '"+f.分公司编号+"', '"+f.分.省+"', '"+f.分.市+"', '','"+f.商户订单号+"', '"+f.设置id+"', '"+f.分设.名称+"', '"+f.上级账号+"', '"+f.上级姓名+"', '"+f.分.层1分公司名称+"', '"+f.分.层1分公司id+"', '"+f.分.层2分公司名称+"', '"+f.分.层2分公司id+"','"+f.分.层3分公司名称+"', '"+f.分.层3分公司id+"', '"+f.总额+"', '"+Number(f.股数)+"', '', '', '', '"+f.分设.每股赠送个数+"', '未支付', '加入独家代理分公司', '系统', '"+时间+"', '', '', '"+f.分公司密码+"');";
